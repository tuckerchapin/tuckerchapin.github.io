...
if (!external_fs_namespaceObject.existsSync('issues')) {
  console.log('No issues/ folder found. Exiting.');
  process.exit();
}

const issueFiles = external_fs_namespaceObject.readdirSync('issues')
for (const issueFile of issueFiles) {
  const content = JSON.parse(external_fs_namespaceObject.readFileSync(`issues/${issueFile}`, 'utf8'))
  if (content.body) {
    const html = `<html><body>${marked(content.body)}</body></html>`;
    if (!external_fs_namespaceObject.existsSync('blog')) external_fs_namespaceObject.mkdirSync('blog');
    external_fs_namespaceObject.writeFileSync(`blog/${issueFile}.html`, html);
